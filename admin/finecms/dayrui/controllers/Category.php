...
class Category extends M_Controller {
    ...
    public function index() {
        ...
        if($id>39){
			...
			$page = $_GET['page'] && $_GET['page']>=1 ? $_GET['page'] : '1';
			//var_dump($page);
			$catlist = $this->db->where('catid', $id)->order_by('updatetime desc')->limit($pagesize,$pagesize * ($page - 1))->get(SITE_ID.'_artical')->result_array();
			$listcount = $this->db->where('catid', $id)->count_all_results(SITE_ID.'_artical');
			$maxpage = ceil($listcount / $pagesize);
